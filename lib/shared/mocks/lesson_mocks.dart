...
class LessonMocks {
  ...
  static Future<List<Map<String, dynamic>>> getSectionsWithProgress(String userId, String courseId) async {
    try {
      ...
      for (final section in sections) {
        ...
        print('📈 Seção ${section['id']}: $completedCount/${lessons.length} lições completadas');
        ...
        if (completedCount == 0) {
          // Se é a primeira seção, está in_progress, senão locked
        ...
        // Desbloquear próxima seção se a anterior foi completada
        if (section['id'] != '1') {
          final previousSectionIndex = int.parse(section['id'] as String) - 2;
          if (previousSectionIndex >= 0 && previousSectionIndex < sections.length) {
            final previousSection = sections[previousSectionIndex];
        ...
        print('  Status final: ${section['status']}');
      }
