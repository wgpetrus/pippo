...
class HomeStatsController extends GetxController {
  ...
  void onInit() {
    ...
    // Inicializar dependências (opcionais)
    try {
    ...
    try {
      ...
      _xpLevelController = null;
    }
    
    try {
      _streakController = Get.find<StreakController>();
    } catch (e) {
      _streakController = null;
    }
  ...
  /// Carrega stats (sincroniza com gamificação)
  void loadStats() {
  ...
  /// Sincroniza com gamificação
  void syncWithGamification() {
    // Sincronização é feita automaticamente pelos controllers de gamificação
    // Este método existe para compatibilidade
  }
